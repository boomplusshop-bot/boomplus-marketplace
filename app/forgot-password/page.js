    ...
    <main style={{minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "linear-gradient(135deg, #eff6ff, #ffffff)", padding: "1rem"}}>
      ...
      <div style={{maxWidth: "440px", width: "100%"}}>
        ...
        <div style={{background: "white", borderRadius: "16px", border: "1px solid #e5e7eb", padding: "2.5rem 2rem", boxShadow: "0 4px 20px rgba(0,0,0,0.08)"}}>
          ...
          {success ? (
            <div>
              <div style={{background: "#f0fdf4", border: "2px solid #bbf7d0", color: "#166534", padding: "1.25rem", borderRadius: "10px", marginBottom: "1.5rem", fontSize: "0.95rem"}}>
                <p style={{margin: 0, fontWeight: "600", marginBottom: "0.5rem"}}>
                  ✅ {success}
                </p>
                <p style={{margin: 0, fontSize: "0.85rem", color: "#15803d"}}>
                  📧 ส่งไปที่: <strong>{sentEmail}</strong>
                </p>
              </div>
              <Link
                href="/login"
                style={{display: "block", textAlign: "center", padding: "0.875rem", background: "#2563eb", color: "white", borderRadius: "10px", textDecoration: "none", fontWeight: "600"}}
              >
                กลับไปเข้าสู่ระบบ
              </Link>
            </div>
            ...
            <form action={handleSubmit}>
              ...
              <button
                type="submit"
                disabled={isLoading}
                style={{width: "100%", padding: "1rem", background: isLoading ? "#93c5fd" : "#2563eb", color: "white", border: "none", borderRadius: "10px", fontSize: "1.05rem", fontWeight: "600", cursor: isLoading ? "not-allowed" : "pointer", boxShadow: "0 2px 8px rgba(37, 99, 235, 0.3)"}}
              >
